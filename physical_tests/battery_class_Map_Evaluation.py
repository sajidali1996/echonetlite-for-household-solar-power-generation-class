...
def save_mapping_pdf_report(n9e_table, n9f_table, n9d_table, n9e_epc, n9e_resp, n9f_epc, n9f_resp, n9d_epc, n9d_resp, filename):
    """
    Generate a PDF report summarizing the Set Property Map (0x9E), Get Property Map (0x9F), and Status change announcement property map (0x9D).

    Args:
        n9e_table (list of dict): Decoded 0x9E property map table.
        n9f_table (list of dict): Decoded 0x9F property map table.
        n9d_table (list of dict): Decoded 0x9D property map table.
        ...
        n9d_epc (int): EPC code for 0x9D.
        n9d_resp (str): Raw response for 0x9D.
    ...
    for row in n9f_table:
        ...
        for i in range(max_lines):
            ...
            pdf.ln()
    pdf.ln(5)
    # Show EPC and response for 0x9D
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(0, 8, 'Status change announcement property map (0x9D) Query/Response:', ln=True)
    pdf.set_font("Arial", size=10)
    pdf.multi_cell(0, 7, f"Query EPC: 0x{n9d_epc:02X}\nResponse: {n9d_resp}")
    pdf.ln(2)
    # Table for 0x9D
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, 'Status change announcement property map (0x9D) Decoded Table', ln=True)
    pdf.set_font("Arial", 'B', 10)
    pdf.cell(40, 10, 'EPC', 1)
    pdf.cell(120, 10, 'Description', 1)
    pdf.ln()
    pdf.set_font("Arial", size=10)
    for row in n9d_table:
        epc = row['EPC']
        desc = row['Description']
        desc_lines = []
        while len(desc) > 60:
...
def main():
    ...
    for epc, label in property_maps:
        ...
        if epc == 0x9F:
            ...
            n9d_resp = resp_hex
            epc_list = parse_property_map(edt, is_9f=False) if edt else []
            n9d_table = describe_epc_list(epc_list)
    now = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    pdf_filename = f'Battery_mapping_evaluation_{now}.pdf'
    save_mapping_pdf_report(n9e_table, n9f_table, n9d_table, n9e_epc, n9e_resp, n9f_epc, n9f_resp, n9d_epc, n9d_resp, pdf_filename)
